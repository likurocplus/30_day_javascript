...
let allLang = countries.flatMap((country)=>{
    return country.languages
})
let setAllLang = new Set(allLang)
let countLang = new Map()
function mostSpokenLanguages(countries, num) {
    for (const lang of setAllLang) {
        let result = allLang.filter(ngonngu=>{
            return ngonngu === lang
        })
        countLang.set(lang, result.length)
    }
    const sortedCount = new Map([...countLang.entries()].sort());
